"""
Readability Module (Phase 1: Data Collection)
가독성 평가 데이터 수집 모듈
"""
...
from app.utils.agents.evaluation.evaluators.evaluator_readability import (
...
async def collect_readability_data(
    ...
    """
    print(f"[Readability Module] Starting data collection for: {title}")
    ...
    try:
        ...
        if isinstance(jargon_result, Exception):
            print(f"[Readability Module] Jargon tool failed: {jargon_result}")
            results["jargon"] = f"평가 실패: {str(jargon_result)}"
        else:
            results["jargon"] = jargon_result
            print(f"[Readability Module] Jargon completed")

        if isinstance(consistency_result, Exception):
            print(f"[Readability Module] Consistency tool failed: {consistency_result}")
            results["consistency"] = f"평가 실패: {str(consistency_result)}"
        else:
            results["consistency"] = consistency_result
            print(f"[Readability Module] Consistency completed")

        if isinstance(grammar_result, Exception):
            print(f"[Readability Module] Grammar tool failed: {grammar_result}")
            results["grammar"] = f"평가 실패: {str(grammar_result)}"
        else:
            results["grammar"] = grammar_result
            print(f"[Readability Module] Grammar completed")

        print(f"[Readability Module] Data collection completed for: {title}")
        return results

    except Exception as e:
        print(f"[Readability Module] Error during data collection: {e}")
        import traceback
        traceback.print_exc()
        return {"error": str(e)}
