"""
Attractiveness Module (Phase 1: Data Collection)
매력도 평가 데이터 수집 모듈
"""
...
from app.utils.agents.evaluation.evaluators.evaluator_attractiveness import (
...
async def collect_attractiveness_data(
    ...
    """
    print(f"[Attractiveness Module] Starting data collection for: {title}")
    ...
    try:
        ...
        if isinstance(content_count_result, Exception):
            print(f"[Attractiveness Module] Content count tool failed: {content_count_result}")
            results["content_count"] = f"평가 실패: {str(content_count_result)}"
        else:
            results["content_count"] = content_count_result
            print(f"[Attractiveness Module] Content count completed")

        if isinstance(content_quality_result, Exception):
            print(f"[Attractiveness Module] Content quality tool failed: {content_quality_result}")
            results["content_quality"] = f"평가 실패: {str(content_quality_result)}"
        else:
            results["content_quality"] = content_quality_result
            print(f"[Attractiveness Module] Content quality completed")

        print(f"[Attractiveness Module] Data collection completed for: {title}")
        return results

    except Exception as e:
        print(f"[Attractiveness Module] Error during data collection: {e}")
        import traceback
        traceback.print_exc()
        return {"error": str(e)}
