"""
Specificity Evaluator (Phase 1: Data Collection)
구체성 평가 데이터 수집 모듈
"""
...
from app.utils.evaluation.llm_functions.specificity import (
...
async def collect_specificity_data(
    ...
    """
    print(f"[Specificity Evaluator] Starting data collection for: {title}")
    ...
    try:
        ...
        if isinstance(responsibility_result, Exception):
            print(f"[Specificity Evaluator] Responsibility tool failed: {responsibility_result}")
            results["responsibility"] = f"평가 실패: {str(responsibility_result)}"
        else:
            results["responsibility"] = responsibility_result
            print(f"[Specificity Evaluator] Responsibility completed")

        if isinstance(qualification_result, Exception):
            print(f"[Specificity Evaluator] Qualification tool failed: {qualification_result}")
            results["qualification"] = f"평가 실패: {str(qualification_result)}"
        else:
            results["qualification"] = qualification_result
            print(f"[Specificity Evaluator] Qualification completed")

        if isinstance(keyword_result, Exception):
            print(f"[Specificity Evaluator] Keyword relevance tool failed: {keyword_result}")
            results["keyword_relevance"] = f"평가 실패: {str(keyword_result)}"
        else:
            results["keyword_relevance"] = keyword_result
            print(f"[Specificity Evaluator] Keyword relevance completed")

        if isinstance(required_fields_result, Exception):
            print(f"[Specificity Evaluator] Required fields tool failed: {required_fields_result}")
            results["required_fields"] = f"평가 실패: {str(required_fields_result)}"
        else:
            results["required_fields"] = required_fields_result
            print(f"[Specificity Evaluator] Required fields completed")

        print(f"[Specificity Evaluator] Data collection completed for: {title}")
        return results

    except Exception as e:
        print(f"[Specificity Evaluator] Error during data collection: {e}")
        import traceback
        traceback.print_exc()
        return {"error": str(e)}
