...
작업 프로세스:
1. 제공된 도구를 사용하여 원본 채용 공고 데이터 로드
2. 원본의 모든 텍스트를 꼼꼼히 읽고 섹션별로 분류
3. JobPostingDetailReport 형식으로 정확하게 변환
...
잘못된 예: ["백엔드 개발 경험", "Java/Python 능숙", "3년 이상"] (배열로 반환 금지)
올바른 예: "- 백엔드 개발 경험 3년 이상 보유하신 분\n- Java 또는 Python을 능숙하게 다루실 수 있는 분"
...
중요 - 출력 타입:
- main_responsibilities: 반드시 문자열(string)로 반환, 개행문자로 구분
- required_qualifications: 반드시 문자열(string)로 반환, 개행문자로 구분  
- preferred_qualifications: 반드시 문자열(string)로 반환, 개행문자로 구분
- tech_stack: 배열(list)로 반환
- tools: 배열(list)로 반환

절대 required_qualifications를 ["항목1", "항목2"] 같은 배열로 반환하지 말고
"- 항목1\n- 항목2\n- 항목3" 같은 문자열로 반환하세요.
...
3. 원본 내용을 요약하거나 간략화
4. 등, 기타 같은 표현으로 생략
5. 구체적인 숫자나 조건 누락 (예: 경력 필요는 안됨, 경력 3년 이상으로 구체적으로)
6. 여러 자격요건을 하나로 합치기

출력 형식:
- 유효한 JSON만 출력
- JSON 앞뒤에 마크다운 코드블록이나 설명 없음
- 정보가 없는 필드는 null로 설정"""
...
async def generate_improved_job_posting_async(
    ...
    try:
        ...
        # 요청 메시지 구성
        user_message = f"""'{json_filename}' 파일의 원본 채용공고를 분석하여 JobPostingDetailReport JSON으로 변환하세요.

작업 순서:
1. load_evaluation_json 도구를 사용하여 '{json_filename}' 파일을 로드
2. 원본 채용공고의 모든 섹션을 꼼꼼히 읽기
3. System Prompt의 필드별 추출 가이드를 따라 정확하게 추출
4. JobPostingDetailReport 형식의 JSON으로 반환

특히 주의할 필드:
- main_responsibilities: 문자열로 반환, 모든 카테고리의 모든 세부업무 포함
- required_qualifications: 문자열로 반환, 모든 자격요건을 개별 항목으로 분리
- preferred_qualifications: 문자열로 반환, 모든 우대사항을 개별 항목으로 분리
...
- 정보 요약이나 생략
- 등으로 생략
- required_qualifications, preferred_qualifications, main_responsibilities를 배열로 반환
...
JSON만 출력하고 다른 설명은 포함하지 마세요."""
        ...
        try:
            ...
            if json_data is None:
                raise json.JSONDecodeError("JSON 추출 실패", output, 0)
