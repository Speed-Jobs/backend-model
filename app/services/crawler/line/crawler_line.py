...
- posted_date: 공고 게시일 (YYYY-MM-DD 형식)
...
def extract_job_detail_from_url(job_url: str, job_index: int, screenshot_dir: Path = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            content_inner = page.query_selector("div.content_inner")
            if content_inner:
                description_text = content_inner.inner_text()
                if description_text:
            ...
            if screenshot_dir:
                try:
                    ...
                    screenshot_filename = f"line_job_{job_id}.png"
            ...
            print(f"  [{job_index}] content_inner 추출 실패, body 전체 사용: {e}")
            job_info["description"] = full_text
...
def run_scrape(
    ...
    outputs = {
        "raw_html": out_dir / "line_raw.html",
        "clean_txt": out_dir / "line_clean.txt",
        "json": out_dir / "line_jobs.json",
    ...
    try:
        ...
        base_url = "https://careers.linecorp.com/ko/jobs/"
        page_num = 1
        all_html_content = []
        ...
        while True:
            ...
            try:
                ...
                for sel in card_selectors:
                    ...
                    if cards.count() > 0:
                        selector = sel
                        print(f"[5/10] 페이지 {page_num}: {cards.count()}개의 카드를 찾았습니다")
                        break
                ...
                if cards and cards.count() > 0:
                    ...
                    for i in range(total_cards):
                        try:
                            ...
                            if href:
                                if href.startswith("/"):
                                    full_url = f"https://careers.linecorp.com{href}"
                                else:
                                    full_url = href
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Line Careers 스크래핑")
    parser.add_argument("--part", default="TECHNOLOGY", help="직군 (기본: TECHNOLOGY)")
    parser.add_argument("--company", default="Line", help="회사 (기본: Line)")
