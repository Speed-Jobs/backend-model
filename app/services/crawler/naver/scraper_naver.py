...
def extract_job_detail_from_url(job_url: str, job_index: int, screenshot_dir: Path = None) -> Dict[str, Any]:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # description 추출 - div.card_title_box + div.detail_wrap 합쳐서 가져오기 (수동)
            ...
            try:
                ...
                print(f"  [{job_index}] card_title_box/detail_wrap 추출 실패, body 전체 사용: {e}")
                job_info["description"] = full_text
...
def run_scrape(
    ...
    with sync_playwright() as p:
        ...
        try:
            # card_link에서 onclick 속성의 annoId 추출
            # <a href="#n" class="card_link" onclick="show('30004094')">
            links = page.query_selector_all("a.card_link[onclick]")

            for link in links:
                onclick = link.get_attribute("onclick")
                if onclick:
                    # show('30004094') 또는 show("30004094") 또는 show(annoId) 형태에서 ID 추출
            ...
            print(f"[5/10] card_link에서 {len(all_job_urls)}개 링크 추출 완료")
