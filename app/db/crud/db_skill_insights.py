...
def get_recent_years_by_company(
    ...
    # SQLAlchemy ORM으로 변경하여 여러 패턴 지원
    five_years_ago = date.today().replace(year=date.today().year - 5)
    ...
    query = db.query(
        ...
        func.extract('year', EFFECTIVE_POSTED_AT).desc()
    ).limit(years)
    
    result = query.all()
    return [int(row.year) for row in result]

def get_skill_frequencies_by_years_and_company(
    db: Session,
    company_patterns: List[str],
    years: List[int],
    top_n: int = 10
...
def get_quarterly_skill_trends_by_company(
    db: Session,
    company_patterns: List[str],
    year: int,
    comparison_year: Optional[int] = None,
