"""
YoY (Year-over-Year) Overheat Index 분석 - 1개월 윈도우

이 스크립트는 전년도 대비 금년도 채용 과열도를 1개월 슬라이딩 윈도우로 분석합니다.

주요 기능:
1. 전체 YoY 과열도 지수 계산
2. 직군(Position)별 YoY 과열도 지수 계산
3. 직무(Industry)별 YoY 과열도 지수 계산
4. 현재 월 기준 1개월 윈도우 비교 (예: 2025-12 → 2025.12 vs 2024.12)
5. YoY 공식을 통한 0-100 점수 산출

계산 공식:
    YoY(t) = min(100, max(0, Ct/Bt-1 × 50))
    - Ct: 현재 년도 1개월 공고 수
    - Bt-1: 전년도 동일 1개월 공고 수
    - 0-100 범위로 cap

사용법:
    python test_06_1month.py [년] [월]

    예시:
    python test_06_1month.py 2025 12
    python test_06_1month.py 2025 6
"""
...
def calculate_yoy_score(current_count: int, previous_count: int) -> int:
    ...
    Args:
        current_count: 현재 년도 1개월 공고 수 (Ct)
        previous_count: 전년도 동일 1개월 공고 수 (Bt-1)
...
def main():
    """메인 실행 함수"""
    print("\n" + "=" * 60)
    print("YoY (Year-over-Year) Overheat Index 분석 - 1개월 윈도우")
    print("=" * 60)
    ...
    print(f"  윈도우: 1개월 슬라이딩")
    print(f"  계산 방식: YoY(t) = min(100, max(0, Ct/Bt-1 × 50))")

    # 1개월 윈도우 계산
    current_start, current_end = calculate_1month_window(target_year, target_month)
    previous_start, previous_end = calculate_1month_window(target_year - 1, target_month)
