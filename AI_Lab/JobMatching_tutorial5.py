# tutorial4까지는 벡터 DB에 채용 json파일을 넣었지만 5번같은 경우 반대로 직무 기술서를 Vector DB에 넣게된다.
# 이에 기반하여, 너무 데이터 수가 너무 적기 때문에, 성능적으로 문제가 발생할 수 있다.
# SK AX의 각 채용공고를 크롤링 하여 직무기술서 상의 industry로 매칭 시킬 수 있다면 성능은 올라가겠지만
# 지금 당장은 어려우니 4주 이후에 생각하는걸로...
...
def extract_skills_from_text(skill_text):
    ...
    # LLM 및 AI 관련 특수 키워드 (대소문자 무시)
    llm_keywords = [
        r'\bLLM\b', r'\bRAG\b', r'\bLoRA\b', r'\bPEFT\b', r'\bQLoRA\b',
        r'\bMLOps\b', r'\bGPU\b', r'\bAPI\b', r'\bRESTful\b',
        r'\bPyTorch\b', r'\bTensorFlow\b', r'\bLangChain\b', r'\bLlamaIndex\b',
        r'\bEmbedding\b', r'\bFine-tuning\b', r'\bQuantization\b',
        r'\bVectorDB\b', r'\bVector DB\b', r'\bDocker\b', r'\bKubernetes\b',
        r'\bHugging Face\b', r'\bTransformers\b', r'\bvLLM\b', r'\bTGI\b',
        r'\bPrompt Engineering\b', r'\bAgent\b', r'\bOrchestration\b',
        r'\bOn-Premise\b', r'\bCloud\b', r'\bAzure\b', r'\bAWS\b', r'\bGCP\b',
        r'\bGPT\b', r'\bClaude\b', r'\bLlama\b', r'\bGemini\b'
    ]

    for keyword_pattern in llm_keywords:
        matches = re.findall(keyword_pattern, skill_text, re.IGNORECASE)
...
print(f"직무기술서 Document 변환 완료: {len(documents)}개")
