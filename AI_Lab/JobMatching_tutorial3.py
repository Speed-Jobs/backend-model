        ...
        if os.path.exists(path):
            with open(path, 'r', encoding='utf-8') as f:
                ...
                if isinstance(jobs, list):
                    ...
                    print(f"⚠ 지원하지 않는 데이터 형식: {os.path.basename(path)}")
                    continue
                loaded_files.append(os.path.basename(path))
                print(f"✓ 파일 로드 성공: {os.path.basename(path)} ({job_count}개)")
        else:
...
print(f"ChromaDB 생성 완료 (저장 경로: {chroma_persist_dir})")
...
def calculate_skill_match_score(job_skills, query_skills):
    ...
    query_skills_set = set([s.lower().strip() for s in query_skills if s])
